...
def decode_blk2line(dwarfinfo):
    ...
    for CU in dwarfinfo.iter_CUs():
        ...
        for entry in lineprog.get_entries():
            ...
            line = cur_state.line
            ret[cur_state.address] = file_name + "_" + str(line)
    return ret
...
def extract_dwarf(filename):
    with open(filename, "rb") as f:
        elffile = ELFFile(f)
        if not elffile.has_dwarf_info():
            print("No Dwarf Found in ", filename)
        else:
            dwarf = elffile.get_dwarf_info()
            addr_to_line = decode_blk2line(dwarf)
    pickle.dump(addr_to_line, open(filename + ".addr.dat", "wb"))
...
if __name__ == '__main__':
    ...
    with open(bin_name, "rb") as f:
        ...
        if not elffile.has_dwarf_info():
            ...
            dwarf = elffile.get_dwarf_info()
            addr_to_line = decode_blk2line(dwarf)
    pickle.dump(addr_to_line, open(bin_name + ".addr.dat", "wb"))
